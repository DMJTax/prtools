        ...
        if len(self.targets) > 0:  # we output a prdataset
            if isinstance(out, prdataset):  # check if we can set featlab:
                if len(self.targets) != out.shape[1]:
                    raise ValueError(
                        "Nr output features does not match nr of feature labels."
                    )
...
def sequentialm(task=None, x=None, w=None):
    ...
    if not isinstance(task, str):
        ...
        if (newm[0].mapping_type == "trained") and (newm[1].mapping_type == "trained"):
            ...
            if (
                (newm[0].shape[1] != 0)
                and (newm[1].shape[0] != 0)
                and (newm[0].shape[1] != newm[1].shape[0])
            ):
    ...
    if task == "init":
        ...
        for i in range(len(flist)):
            ...
            if w.hyperparam[0] == "mean":
                ...
                raise ValueError(
                    "Combining '%s' is *not* defined for fixedcc." % w.hyperparam
                )
